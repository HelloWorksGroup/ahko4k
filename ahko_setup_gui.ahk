...
ahko_setup.Add("Text", "x10 y10", "ahko4k setup")
...
ahko_setup.Add("Link", "xs y+0 w" clientWidth " right", 'Visit <a href="https://github.com/HelloWorksGroup/ahko4k">GitHub Page</a>')
...
ahko_setup_show(*) {
	...
	; hotkeyText_update()
	local autostart:=RegRead("HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run","ahko4k","")
...
;@Ahk2Exe-IgnoreEnd
	...
	if(b) {
		runwait(A_Temp "\ahko_temp\startup.exe --name=ahko4k --target=" A_ScriptFullPath)
	} else {
		runwait(A_Temp "\ahko_temp\startup.exe --name=ahko4k --remove")
	}
	try {
